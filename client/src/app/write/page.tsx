...
function WritePageContent() {
  ...
  const [isEditMode, setIsEditMode] = useState(false); // Only true when editing from calendar
  const [isSaved, setIsSaved] = useState(false); // True when current journal has been saved
  ...
  useEffect(() => {
    if (editJournalId && existingJournal.data && !isEditMode) {
      ...
      setIsEditMode(true); // This means we came from calendar to edit
      setIsSaved(true); // This journal already exists, so it's been saved
      setCurrentJournalId(editJournalId);
      setEditorKey(previous => previous + 1); // Force editor re-render with new content
  ...
  const handleResetJournal = () => {
    ...
    // Reset the journal text to template content and force editor re-render
    setJournalText(content);
    setEditorKey(previous => previous + 1); // Force editor to re-render with new content

    // Only navigate if we have an editJournalId (came from calendar)
    if (editJournalId) {
      router.push('/write');
    }
  };
    ...
    <div className="container mx-auto max-w-7xl px-4 py-8">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between px-6">
          {/* Back to Calendar button - left side when editing */}
          ...
          {/* Reset Journal Button - right side */}
          <div className="flex items-center">
            <Button
              variant="outline"
              size="sm"
