...
export function TipTapFloatingMenu({ editor }: { editor: Editor }) {
  ...
  const executeCommand = useCallback(
    (commandFunction: (editor: Editor) => void) => {
      if (!editor) return;
      ...
      switch (event_.key) {
        case 'ArrowDown': {
          ...
          setSelectedIndex(previous => {
            ...
            return previous < flatFilteredItems.length - 1 ? previous + 1 : 0;
          });
          break;
        }
        ...
        case 'ArrowUp': {
          preventDefault();
          setSelectedIndex(previous => {
            if (previous === -1) return flatFilteredItems.length - 1;
            return previous > 0 ? previous - 1 : flatFilteredItems.length - 1;
          });
          break;
        }
  ...
  useEffect(() => {
    ...
    const handleEditorKeyDown = (event_: Event) =>
      handleKeyDown(event_ as KeyboardEvent);
  ...
  useEffect(() => {
    if (selectedIndex >= 0 && itemReferences.current[selectedIndex]) {
      itemReferences.current[selectedIndex]?.focus();
    }
  }, [selectedIndex]);
                ...
                {group.items.map((item, itemIndex) => {
                  const flatIndex =
                    filteredGroups
                      .slice(0, groupIndex)
                      .reduce(
                        (accumulator, g) => accumulator + g.items.length,
                        0,
                      ) + itemIndex;
                  ...
                  return (
                    <CommandItem
                      ...
                      ref={element => {
                        itemReferences.current[flatIndex] = element;
                      }}
